...
namespace GameOne
{
    class GlitchPlayer : UserControlledSprite
    {
        ...
        static Vector2 glitchSpeed = new Vector2(8,4);
        ...
        public GlitchPlayer(Texture2D image)
            : base(new SpriteSheet(image, glitchNumberOfFrames, 1f),
           new Vector2(2000f, 2000f), glitchCollisionOffset, glitchSpeed, glitchFriction)
        {
        ...
        public override void Collision(Platform otherSprite)
        {
            float spritePosition = otherSprite.position.Y - otherSprite.frameSize.Y;
            float scaleFactor = this.spriteSheet.scale;
            float left = position.X + (glitchFrameSize.X + collisionOffset.east) * scaleFactor;
            float right = position.X - collisionOffset.west;
            float top = position.Y + (glitchFrameSize.Y - collisionOffset.south) * scaleFactor;
            Boolean topCorners = glitchFrameSize.Y * scaleFactor + position.Y < otherSprite.position.Y + glitchFrameSize.Y * scaleFactor;

            if (top > otherSprite.position.Y + otherSprite.coll.north && topCorners)
            {
                velocity.Y = 0;
                onGround = true;
                position.Y = otherSprite.position.Y + otherSprite.coll.north - glitchFrameSize.Y * scaleFactor;

            }

            else if (position.Y - collisionOffset.north * scaleFactor <= otherSprite.position.Y + otherSprite.frameSize.Y)
            {
                velocity.Y = 0;
                position.Y = otherSprite.position.Y + otherSprite.frameSize.Y - collisionOffset.north;
            }

            else if (left >= otherSprite.position.X && left <= otherSprite.position.X + otherSprite.frameSize.X / 2)
            {
                velocity.X = 0;
                position.X = otherSprite.position.X - (glitchFrameSize.X - collisionOffset.east) * scaleFactor;
            }
            else if (right <= otherSprite.position.X + otherSprite.frameSize.X && right >= otherSprite.frameSize.X / 2)
            {
                velocity.X = 0;
                position.X = otherSprite.position.X - collisionOffset.west * scaleFactor + otherSprite.frameSize.X;
            }
